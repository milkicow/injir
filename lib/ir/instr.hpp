...
public:
  explicit BranchInstr(const Value *cond, const BasicBlock *true_bb,
                       const BasicBlock *false_bb)
      : Instr(InstrType::kBranch), m_cond(cond), m_true_bb(true_bb),
        m_false_bb(false_bb) {}
};
...
public:
  explicit ReturnInstr(const Value *ret)
      : Instr(InstrType::kReturn), m_ret(ret) {}
};
