...
#include "cfg_analysis/lifetime.hpp"
...
static void
check_lifetime(const cfg_analysis::LifeTime &&lifetime,
               std::unordered_map<Instr *, cfg_analysis::LifeTime::life_ranges_t> &&expected) {

    for (auto &&[instr, intervals] : expected) {
        auto exact = lifetime.get_lifetime(instr);
        EXPECT_EQ(exact.size(), intervals.size());

        auto proj = &cfg_analysis::LifeTime::life_range_t::first;
...
static void print_lifetime(const cfg_analysis::LifeTime &lifetime,
    ...
    for (const auto &bb : order) {
        ...
        for (const auto &instr : *bb) {
            std::cout << "  Instr: " << std::hex << instr.get() << " | [" << std::dec << time << " "
                      << time + cfg_analysis::LifeTime::kLifetimeStep << "]" << std::dec;
            print_lifetime("", instr.get());
            time += cfg_analysis::LifeTime::kLifetimeStep;
        }
    }
}

TEST_F(CFGLifeTimeSimpleExample, LIFETIME) {
    cfg_analysis::LifeTime lifetime{bb_a, cfg_analysis::loop_tree(bb_a), basic_block_counter};
    auto order = detail::linear_order(bb_a, cfg_analysis::loop_tree(bb_a), basic_block_counter);

    std::vector<BasicBlock *> expected{bb_a, bb_b};
    EXPECT_TRUE(std::equal(order.begin(), order.end(), expected.begin()));

    std::unordered_map<Instr *, cfg_analysis::LifeTime::life_ranges_t> expected_lifetimes{
...
TEST_F(CFGLifeTimePaperExample, LIFETIME) {
    ...
    cfg_analysis::LifeTime lifetime{bb_a, loop_tree, basic_block_counter};
